...
With only socks and shoes, there are 16 actions, so we would have $16!$ in the numerator. This count would allow for putting on shoes before socks which we will have to fix. For each of the eight tentacles, there are two orders (socks then shoes, or shoes then socks), so we divide be $2!$ for each tentacle, which gives $(2!)^8$ in denominator.

$$\text{Number of Permutations} = \frac{16!}{(2!)^8}\approx 8.17 \times 10^{10}$$

With socks, shoes, and tying, there are now 24 actions (putting on a sock, a shoe, and tying the shoe for each of the 8 tentacles) so we have $24!$ in the numerator. Similar to earlier, we have to get rid of any permutation that would have Mr. Octopus putting on a shoe before a sock or tying his shoe before even putting it on. Since there are $3!$ orderings for these three actions on a tentacle and only one of them is correct, we would divide by $3!$ for each tentacle, giving $(3!)^8$ in the denominator.
